		...
		<div class="row">
			...
			<div class="col-lg-3 col-md-12 col-sm-12 col-12 d-none d-sm-block">
				...
				<div class="categorysidemenu bg-dark p-3">
					...
					<li>
						<a href="{{url('subcategory')}}/{{ str_replace(' ','-',$cat->subcategory_name) }}/{{$cat->id}}" class="text-light"><span uk-icon="icon: chevron-right; ratio: 1"></span>&nbsp;{{ $cat->subcategory_name }}</a>
					</li>
					@endforeach
					@endif
			...
			<div class="col-lg-9 col-md-12 col-sm-12 col-12">
				...
				<div>
					...
					@else
							...
							<div class="p-5" style="background:linear-gradient(0deg, rgba(0, 0, 0, 0.3), rgba(0, 0, 0, 0.3)), url('http://halalbuy.com.bd/public/categoryImage/{{$category->banner}}');background-position: center; background-size: cover;background-repeat: no-repeat; ">
					    
					    <h4 class="text-center text-light font-weight-bold text-uppercase">{{$category->category_name}}</h4>
			...
			$("#brand_id:checked").change(function(){
				...
				if(id != '')
				{
					...
					$.ajax({
						...
						url: '{{ url("brand_wise_search_category") }}',
						type: 'POST',
