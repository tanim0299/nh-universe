		...
		<div class="row">
			...
			<div class="col-lg-3 col-md-12 col-sm-12 col-12 d-none d-sm-block">
				...
				<div class="categorysidemenu bg-dark p-3">
					...
					<h5 class="head">Category&nbsp;<span uk-icon="icon:  chevron-down; ratio: 1"></span></h5>
					@if($category)
					@foreach($category as $cat)
					<li>
						<a href="{{url('category')}}/{{ str_replace(' ','-',$cat->category_name) }}/{{$cat->id}}" class="text-light"><span uk-icon="icon: chevron-right; ratio: 1"></span>&nbsp;{{ $cat->category_name }}</a>
					</li>
					@endforeach
					@endif
			...
			<div class="col-lg-9 col-md-12 col-sm-12 col-12">
				...
				<div>
					@if($item->banner)
					<div class="p-5" style="background:linear-gradient(0deg, rgba(0, 0, 0, 0.3), rgba(0, 0, 0, 0.3)), url('http://halalbuy.com.bd/public/itemImage/{{$item->banner}}');background-position: center; background-size: cover;background-repeat: no-repeat; ">
					    
					    <h4 class="text-center text-light font-weight-bold text-uppercase">{{$item->item_name}}</h4>
					...
					<ul class="uk-breadcrumb">
						...
						<li><span>{{$item->item_name}}</span></li>
					</ul>
         ...
         @endif
							 ...
							 <div class="homeproducts border">
								<center>
									<a href="{{url('product')}}/{{$productname}}/{{$s->product_id}}"><img src="{{asset('public/productImage')}}/{{$s->image}}" class="img-fluid" style="z-index:1;"></a>
								</center>
			...
			$("#brand_id:checked").change(function(){
				...
				if(id != '')
				{
					...
					$.ajax({
						...
						url: '{{ url("brand_wise_search_item") }}',
						type: 'POST',
			...
			$("#color:checked").change(function(){
				...
				if(id != '')
				{
					...
					$.ajax({
						...
						headers: { 'X-CSRF-TOKEN': "{{ csrf_token() }}" },
						url: '{{ url("color_wise_search_item") }}',
						type: 'POST',
						data: {id:id,item_id:item_id},
						success: function(data)
						{
			...
			$("#price:checked").change(function(){
				...
				if(id != '')
				{
					...
					$.ajax({
						...
						headers: { 'X-CSRF-TOKEN': "{{ csrf_token() }}" },
						url: '{{ url("price_wise_search_item") }}',
						type: 'POST',
						data: {id:id,item_id:item_id},
						success: function(data)
						{
